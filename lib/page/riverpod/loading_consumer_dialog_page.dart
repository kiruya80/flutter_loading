...
class LoadingConsumerDialogPage extends ConsumerStatefulWidget {
  @override
  ConsumerState<LoadingConsumerDialogPage> createState() =>
      _LoadingConsumerDialogPageState();
}

class _LoadingConsumerDialogPageState
    extends RouteConsumerState<LoadingConsumerDialogPage> {
  bool isCanPop = false;
  ...
  Widget build(BuildContext context) {
    ...
    // QcLog.d('LoadingConsumerDialogPage ===  ${navigatorObserver.routeStack.toString()}');
    // QcLog.d('routeStack length ===  ${navigatorObserver.routeStack.length}');

    return Scaffold(
      appBar: AppBar(title: Text("Loading ConsumerDialog Page")),
        ...
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              // Navigator.pushNamed(context, '/ThirdConsumerPage', arguments: isCanPop);

              Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) => ThirdConsumerPage(
                            isLoading: isMoveLoading,
                          ),
                      settings: RouteSettings(name: 'ThirdConsumerPage')));
              // await Future.delayed(const Duration(seconds: 5));
            ...
            child: Text(
                "${isCanPop ? "백키 가능" : "백키 불가"} , 로딩 5초 후 ThirdConsumerPage 이동"),
          ),
